          ...
          {resultUrl && (
            <div>
              <div className="mb-1.5 text-xs font-medium uppercase tracking-wide text-[var(--ink-dim)]">
                Result — {(resultSize / 1024 / 1024).toFixed(2)} MB
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={resultUrl} alt="Upscaled" className="max-h-64 w-full rounded-md border border-[var(--ground-line)] object-contain" />
              <a href={resultUrl} download={`upscaled.${ext}`} className="mt-2 inline-flex items-center gap-1.5 rounded-md bg-[var(--gold)] px-3 py-1.5 text-xs font-medium text-[#0a0c0d] hover:bg-[var(--gold-dim)]">
