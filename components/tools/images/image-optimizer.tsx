          ...
          {resultUrl && (
            <div>
              <div className="mb-1.5 text-xs font-medium uppercase tracking-wide text-[var(--ink-dim)]">
                Result — {fmt(resultSize)} ({originalSize > 0 ? Math.round((1 - resultSize / originalSize) * 100) : 0}% smaller)
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={resultUrl} alt="Result" className="max-h-56 w-full rounded-md border border-[var(--ground-line)] object-contain" />
              <a
                href={resultUrl}
                download={`optimized.${ext}`}
