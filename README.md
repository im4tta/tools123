...
>
> - 2026-07-30: Added new 3D model miniatures to the Model Gallery and improved the gallery UI. Newly-registered models include Angkor Wat and Bayon (earlier), plus six additional Khmer temple miniatures: Wat Phnom, Ta Prohm, Banteay Srei, Preah Vihear, Koh Ker, and Sambor Prei Kuk. Models are provided as dynamic model-factory modules and appear in the viewer's preview list.
> - 2026-07-30: Materials tool UI fixes — the materials page header background and borders were adjusted to align with the main content width and an extra underline under the header menu was removed for a cleaner appearance.
> - 2026-07-30: 3D Model Gallery — replaced the model dropdown with a horizontal preview gallery and added a short note on pages that several model factory files were generated by an AI assistant; some meshes or shapes may need manual refinement.
> - 2026-07-30: Synthesizer / Crafting Lab improvements — the element picker now shows all elements (not just the first 20), and in-context hints are shown when the first ingredient is selected (suggested second ingredients and known 3-input recipes). This reduces the number of "Unknown Reaction" fallbacks caused by unavailable inputs. The UI still shows two input slots; adding a third slot is available on request.
> - 2026-07-30: Khmer translations in the Materials fun-facts were corrected for clarity.
>
